...
type ResolveExistingObligationFailure = { ok: false; error: string };
type LifecycleRow = { lifecycle_state: string; entity_id: string | null; receipt_entity_id: string | null };
type ApiResponse = ResolveExistingObligationSuccess | ResolveExistingObligationFailure;
...
function requireTrimmedString(value: unknown, errorCode: string): string {
  if (typeof value !== "string" || !value.trim()) throw new RequestValidationError(errorCode);
  return value.trim();
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  try {
    ...
    const requestBody = req.body && typeof req.body === "object" && !Array.isArray(req.body) ? (req.body as Record<string, unknown>) : null;
    if (!requestBody) throw new RequestValidationError("REQUEST_BODY_INVALID");
    ...
    const userSupabase = createServerClient(url, anonKey, {
      cookies: {
        ...
        set(name, value, options) { appendSetCookie(res, serialize(name, value, { path: "/", ...options })); },
        remove(name, options) { appendSetCookie(res, serialize(name, "", { path: "/", maxAge: 0, ...options })); },
      },
    });
    const serviceSupabase = createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } }) as SupabaseServiceClient;
    ...
    if (!obligation) return res.status(404).json({ ok: false, error: "OBLIGATION_NOT_FOUND" });
    ...
    if (!userError && user) {
      ...
      if (membershipError) return res.status(500).json({ ok: false, error: `MEMBERSHIP_LOOKUP_FAILED: ${membershipError.message}` });
    ...
    const { data: proofRaw, error: proofError } = await serviceSupabase.schema("api").rpc("evaluate_proof_boundary", {
      p_obligation_id: obligationId,
      p_actor_id: actorId,
      p_decision: "approve",
      p_evaluation_mode: "policy_evaluated",
      p_rationale: {
      ...
      p_cited_controls: ["system-proof-board-v1"],
      p_required_follow_up: [],
      p_evidence_snapshot: {
        source: "system-proof-board",
    ...
    const proofBoundary = asRecord(proofRaw, "PROOF_BOUNDARY_RESULT");
    const resolution = asRecord(proofBoundary.resolution, "RESOLUTION_RESULT");
    const receiptId = resolution.receipt_id;
    if (typeof receiptId !== "string" || !receiptId.trim()) throw new Error("RESOLUTION_RESULT_INCOMPLETE");

    const [obligationResult, receiptResult, lifecycleResult] = await Promise.all([
      serviceSupabase.schema("core").from("obligations").select("*").eq("id", obligationId).single(),
      serviceSupabase.schema("receipts").from("receipts").select("*").eq("id", receiptId).single(),
      serviceSupabase.schema("projection").from("obligation_lifecycle").select("lifecycle_state, entity_id, receipt_entity_id").eq("obligation_id", obligationId).single(),
    ]);

    const queryError = obligationResult.error || receiptResult.error || lifecycleResult.error;
