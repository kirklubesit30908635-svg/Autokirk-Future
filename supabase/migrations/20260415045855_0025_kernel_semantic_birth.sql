...
alter table core.obligations
  alter column obligation_code set default 'unclassified';

update core.obligations
set obligation_code = 'unclassified'
where obligation_code = 'legacy_obligation';
...
begin
    ...
    v_truth_burden :=
        case
            when p_source_system = 'stripe'
             and p_source_event_key in ('invoice.paid', 'charge.succeeded')
                then 'performance'
            else 'promise'
        end;
    ...
    v_obligation_code :=
        case
            when p_source_system = 'stripe'
             and p_source_event_key = 'invoice.upcoming'
